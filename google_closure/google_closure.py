...
def configure(ctx):
    ...
    tools_node = ctx.path.find_dir(os.path.relpath(tooldir))
    if not tools_node:
        ctx.fatal('Unable to locate the google_closure tool directory.')
    ctx.env.CLOSURE_TOOLS = tools_node.relpath()
    ...
    ctx.env.CLOSURE_LIBRARY = closure_library_node.relpath() 
...
Note: Did you run `git submodule init` and `git submodule update`''')
    ctx.env.CLOSURE_SCRIPTS = closure_scripts_node.relpath()
